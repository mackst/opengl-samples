...
class MyGLWidget(QGLWidget):
    ...
    def initializeGL(self):
        # check if support 4.3
        majorVersion = glGetInteger(GL_MAJOR_VERSION)
        minorVersion = glGetInteger(GL_MINOR_VERSION)
        if majorVersion < 4 or minorVersion < 3:
            raise Exception("Compute Shader only support on OpenGL 4.3.")
        
        glClearColor(0, 0, 0, 0)
        
        # create compute shader
        comshader = shaderFromFile(GL_COMPUTE_SHADER, 'invert.glsl')
        self.comprogram = shaders.compileProgram(comshader)
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, self.im.size[0], self.im.size[1], 
                     0, GL_RGBA, GL_UNSIGNED_BYTE, im.tostring())
        
        self.resultTexture = glGenTextures(1)
        glBindTexture(GL_TEXTURE_2D, self.resultTexture)
        glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, self.im.size[0], self.im.size[1])
    ...
    def runComputeFilter(self, inputTex, outputTex):
        glUseProgram(self.comprogram)
        ...
        glDispatchCompute(self.im.size[0] / self.workgroupSize, self.im.size[1] / self.workgroupSize, 1)
    ...
    def paintGL(self, *args, **kwargs):
        ...
        if self.enableFilter:
            self.runComputeFilter(self.textureID, self.resultTexture)
            glBindTexture(GL_TEXTURE_2D, self.resultTexture)
        else:
            glBindTexture(GL_TEXTURE_2D, self.textureID)
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Space:
            self.enableFilter = not self.enableFilter
            self.updateGL()
            #sys.stdout.write("Filter: %s\n" % str(self.enableFilter))
        return super(MyGLWidget, self).keyPressEvent(event)
